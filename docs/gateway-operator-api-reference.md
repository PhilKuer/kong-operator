...
| `workloadType` _[WorkloadType](#common-konghq-com-v1alpha1-types-workloadtype)_ | WorkloadType selects the Kubernetes workload that runs the DataPlane's Pods.<br /><br />`Deployment` (the default) runs the configured number of replicas. `DaemonSet` runs exactly one DataPlane Pod on every eligible node.<br /><br />The `replicas`, `scaling` and `rollout` fields cannot be used with `DaemonSet` because a DaemonSet's size follows the number of eligible nodes. |
