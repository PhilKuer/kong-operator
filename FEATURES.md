...
By default a `DataPlane`'s `Pods` are run by a `Deployment` with the configured
number of replicas. Setting `spec.deployment.workloadType: DaemonSet` on a
`DataPlane` - or `spec.dataPlaneOptions.deployment.workloadType: DaemonSet` on a
